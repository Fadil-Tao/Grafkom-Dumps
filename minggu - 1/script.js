...
function cerminCekung(imageData, xc, yc, radiusX, radiusY, r, g, b) {
  for (let theta = 0; theta < Math.PI * 2; theta += 0.0001) {
    x = xc + radiusX * Math.cos(theta / 4 - (1 / 4) * Math.PI);
    y = yc + radiusY * Math.sin(theta / 4 - (1 / 4) * Math.PI);

    console.log(`cermin cekung's pixel location x:${x} y:${y}`)
    DrawLine(imageData, Math.ceil(x), Math.ceil(y), r, g, b);
  }
}


// Garis X yang ijo dari ujung ke ujung
DDA(imageData, 0, 300, 100, 100, 0, 255, 0);
...
// Garis Cermin CEKUNG
cerminCekung(imageData, 100, 100, 50, 50, 255, 0, 0);
